...
async function main() {
  ...
  const modalsStart = code.indexOf('{showCreateAdmin && (');
  // Finding end of the three modals is harder, let's just use string replacement on a large chunk.
  
  // For safety with regex replacement, we can do:
  code = code.replace(
      /\{showCreateAdmin && \([\s\S]*?\{showCreatePm && \([\s\S]*?<\/div>[\s]*<\/div>[\s]*\)\}/,
\`{showCreateUser && (
